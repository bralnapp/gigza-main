export const reviews = Array(4).fill({
	noOfReviews: Math.floor(Math.random() * 5) + 1,
	gig: "We're a startup company that need a UI designer.",
	clientName: "raphael Benjamin",
	date: "Tue Oct 06 2020",
	review:
		"We're a startup company that need a UI designer. I found Ofspace on Dribbble. We reviewed Ofspace's previous works and found his works are amazing! After we worked with Ofspace, we realized they ha..."
});

export const recentJobs = [
	{
		jobTitle: "Graphics design",
		jobDescription:
			"I'm looking for someone who can handle a logo designing project. If you are interested, kindly send a message.",
		skills: ["logo design", "Brand/graphics design"],
		client: "raphael Benjamin",
		time: "3 hours ago",
		budget: 250
	},
	{
		jobTitle: "Tech company pitch deck",
		jobDescription:
			"I work for a tech start-up called MODCLUB, and we are looking to have our 15-page investor deck refreshed by a professional designer. We are looking for a clean and simple final product that is not overly designed. Timeline- asap.",
		skills: ["Brand/graphics design"],
		client: "raphael Benjamin",
		time: "3 hours ago",
		budget: 500
	},
	{
		jobTitle: "UI app design",
		jobDescription:
			"Hi, I'm looking for a designer to help me create 20 screen UI designs for Android and iOS. Kindly send your portfolio and your quote for this job. Thanks!",
		skills: ["mobile design", "UI/UX design"],
		client: "raphael Benjamin",
		time: "3 hours ago",
		budget: 1500
	}
];

export const sentProposals = [
	{
		title: "Freelance UI/UX Designer (Mobile app)",
		description:
			"I work for a tech start-up called MODCLUB, and we are looking to have our 15-page investor deck refreshed by a professional designer. We are looking for a clean and simple final product that is not overly designed. Timeline- asap.",
		name: "Roger Siphron",
		date: "Jun 17, 2022",
		status: "open"
	},
	{
		title: "UX/UI Designer for cryptocurrency gambling website.",
		description:
			"I'm looking for someone who can handle a logo designing project. If you are interested, kindly send a message.",
		name: "Chance Donin",
		date: "Jun 17, 2022",
		status: "closed"
	},
	{
		title: "Design the UI for an online B2C health platform",
		description:
			"Hi, I'm looking for a designer to help me create 20 screen UI designs for Android and iOS. Kindly send your portfolio and your quote for this job. Thanks!",
		name: "Maria Dorwart",
		date: "Jun 17, 2022",
		status: "closed"
	},
	{
		title: "Website in webflow",
		description:
			"We have a branding package and need a website built in webflow. Looking for someone with a design eye who can help make our branding package come to life.",
		name: "Tiana Dias",
		date: "Jun 17, 2022",
		status: "open"
	},
	{
		title: "Tech company pitch deck",
		description:
			"I work for a tech start-up called MODCLUB, and we are looking to have our 15-page investor deck refreshed by a professional designer. We are looking for a clean and simple final product that is not overly designed. Timeline- asap.",
		name: "Cooper Kenter",
		date: "Jun 17, 2022",
		status: "closed"
	}
];

export const contracts = {
	active: [
		{
			title: "Tech company pitch deck",
			description:
				"I work for a tech start-up called MODCLUB, and we are looking to have our 15-page investor deck refreshed by a professional designer. We are looking for a clean and simple final product that is not overly designed. Timeline- asap.",
			endDate: "Sept 21, 2022",
			startDate: "Jun 17, 2022",
			name: "Davis Herwitz"
		},
		{
			title: "Graphics designs",
			description:
				"I'm looking for someone who can handle a logo designing project. If you are interested, kindly send a message.",
			endDate: "Sept 21, 2022",
			startDate: "Jun 17, 2022",
			name: "Leo Torff"
		},
		{
			title: "UI app design",
			description:
				"Hi, I'm looking for a designer to help me create 20 screen UI designs for Android and iOS. Kindly send your portfolio and your quote for this job. Thanks!",
			endDate: "Sept 21, 2022",
			startDate: "Jun 17, 2022",
			name: "Martin Vaccaro"
		},
		{
			title: "Website in webflow",
			description:
				"We have a branding package and need a website built in webflow. Looking for someone with a design eye who can help make our branding package come to life.",
			endDate: "Sept 21, 2022",
			startDate: "Jun 17, 2022",
			name: "Alfonso Botosh"
		},
		{
			title: "Tech company pitch deck",
			description:
				"I work for a tech start-up called MODCLUB, and we are looking to have our 15-page investor deck refreshed by a professional designer. We are looking for a clean and simple final product that is not overly designed. Timeline- asap.",
			endDate: "Sept 21, 2022",
			startDate: "Jun 17, 2022",
			name: "Kadin Baptista"
		}
	],
	closed: [
		{
			title: "Basic Webpage and Multifunctional MVP wireframe",
			description:
				"I work for a tech start-up called MODCLUB, and we are looking to have our 15-page investor deck refreshed by a professional designer. We are looking for a clean and simple final product that is not overly designed. Timeline- asap.",
			status: "awaiting payment",
			name: "raphael Benjamin",
			startDate: "Jun 17, 2022"
		},
		{
			title: "UX/UI designer - Long term",
			description:
				"I'm looking for someone who can handle a logo designing project. If you are interested, kindly send a message.",
			status: "completed",
			name: "raphael Benjamin",
			startDate: "Jun 17, 2022"
		},
		{
			title: "UI app design",
			description:
				"Hi, I'm looking for a designer to help me create 20 screen UI designs for Android and iOS. Kindly send your portfolio and your quote for this job. Thanks!",
			status: "completed",
			name: "raphael Benjamin",
			startDate: "Jun 17, 2022"
		},
		{
			title: "Website in webflow",
			description:
				"We have a branding package and need a website built in webflow. Looking for someone with a design eye who can help make our branding package come to life.",
			status: "completed",
			name: "raphael Benjamin",
			startDate: "Jun 17, 2022"
		},
		{
			title: "Tech company pitch deck",
			description:
				"I work for a tech start-up called MODCLUB, and we are looking to have our 15-page investor deck refreshed by a professional designer. We are looking for a clean and simple final product that is not overly designed. Timeline- asap.",
			status: "completed",
			name: "raphael Benjamin",
			startDate: "Jun 17, 2022"
		}
	],
	pending: [
		{
			title: "Tech company pitch deck",
			description:
				"I work for a tech start-up called MODCLUB, and we are looking to have our 15-page investor deck refreshed by a professional designer. We are looking for a clean and simple final product that is not overly designed. Timeline- asap.",
			status: "pending",
			name: "raphael Benjamin",
			startDate: "Jun 17, 2022"

		}
	]
};
